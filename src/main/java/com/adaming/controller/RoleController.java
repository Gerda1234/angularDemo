...
@CrossOrigin("*")// n'importe quel port peut communiquer avc le port 8080)
@RestController
public class RoleController {
	...
	public Role updateRole(@PathVariable("id") Long id, @RequestBody Role role) {
		Role currentUser = roleService.findOne(id);
		currentUser.setNomRole(role.getNomRole());
		
		return roleService.save(currentUser);
	}
}
